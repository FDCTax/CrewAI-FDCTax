...
export async function sendGSTFollowUpEmail(clientData) {
  try {
    const { data, error } = await resend.emails.send({
      from: `${process.env.RESEND_FROM_NAME} <${process.env.RESEND_FROM_EMAIL}>`,
      to: [clientData.email],
      subject: 'Next steps for your GST registration',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <style>
            ...
            .button { display: inline-block; padding: 12px 24px; background: #15ADC2; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; }
            .footer { text-align: center; padding: 20px; color: #6b7280; font-size: 14px; }
            .info-box { background: #f0f9ff; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #15ADC2; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin: 0;">GST Registration Support</h1>
              <p style="margin: 10px 0 0 0; opacity: 0.9;">Let's help you get set up</p>
            </div>
            <div class="content">
              <p>Hi ${clientData.casual_name || clientData.first_name},</p>
              
              <p>Thank you for indicating your interest in GST registration assistance. We're here to help make this process simple and straightforward for you.</p>
              
              <div class="info-box">
                <h3 style="margin-top: 0; color: #15ADC2;">What happens next?</h3>
                <p style="margin-bottom: 0;">One of our experienced tax professionals will contact you within 1-2 business days to:</p>
                <ul style="margin: 10px 0 0 0;">
                  <li>Discuss your specific situation</li>
                  <li>Explain GST requirements for your FDC business</li>
                  <li>Help you understand if GST registration is right for you</li>
                  <li>Assist with the registration process if needed</li>
                </ul>
              </div>
              
              <h3>📄 Helpful Resource</h3>
              <p>In the meantime, you might find this fact sheet helpful:</p>
              <p>
                <a href="https://fdctax.sharepoint.com/:b:/s/FDCTaxTeam/EeS8vK9uN9pGvq8jKQx_n8IBpYqx-1qQMJxXO_4vZqOKBw?e=1HU0qn" 
                   class="button" 
                   style="color: white;">
                  Download: Should I Register for GST?
                </a>
              </p>
              
              <h3>📞 Need to speak sooner?</h3>
              <p>Feel free to call us anytime on <strong>1300 000 000</strong>. We're here Monday to Friday, 9am-5pm AEST.</p>
              
              <p style="margin-top: 30px;">We look forward to helping you with your GST registration!</p>
              
              <p>Warm regards,<br>
              <strong>The FDC Tax Team</strong></p>
            </div>
            <div class="footer">
              <p>FDC Tax Pty Ltd | hello@fdctax.com.au<br>
              ABN: [Your ABN] | www.fdctax.com.au</p>
              <p style="font-size: 12px; color: #9ca3af; margin-top: 10px;">
                This email was sent because you requested GST registration assistance during your onboarding.
              </p>
            </div>
          </div>
        </body>
    ...
    if (error) {
      console.error('Resend error (GST follow-up):', error)
      return { success: false, error }
    }

    console.log('✅ GST follow-up email sent:', data)
    return { success: true, data }
  } catch (error) {
    console.error('Error sending GST follow-up email:', error)
    return { success: false, error: error.message }
  }
}
