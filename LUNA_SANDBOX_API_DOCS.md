...
```
https://fdcinsight.preview.emergentagent.com
```
...
**Example Request:**
```bash
curl "https://fdcinsight.preview.emergentagent.com/api/user/context?user_id=143000"
```
...
**Example Request:**
```bash
curl "https://fdcinsight.preview.emergentagent.com/api/user/checklist?user_id=143000"
```
...
**Example Request:**
```bash
curl -X POST "https://fdcinsight.preview.emergentagent.com/api/user/checklist/update" \
...
**Example Request:**
```bash
curl "https://fdcinsight.preview.emergentagent.com/api/user/bas-status?user_id=143001"
```
...
**Example Request:**
```bash
curl -X POST "https://fdcinsight.preview.emergentagent.com/api/luna-rag/chat" \
  -H "Content-Type: application/json" \
  -d '{
...
export default function EducatorDashboard() {
  ...
  useEffect(() => {
    async function loadContext() {
      const res = await fetch(
        `https://fdcinsight.preview.emergentagent.com/api/user/context?user_id=${userId}`
      );
  ...
  async function generateGreeting(context) {
    const res = await fetch(
      'https://fdcinsight.preview.emergentagent.com/api/luna-rag/chat',
      {
        method: 'POST',
